...
As a person trying to lose weight, I want the app to ask for the basic personal
details needed to estimate my daily calorie budget on first use, so I can start
tracking against a goal without manual research or setup complexity.
...
**Independent Test**: A new user can complete onboarding by entering the required
personal details, see an estimated daily calorie budget, and reopen the app later
to see that the budget is still being used for daily tracking.

**Acceptance Scenarios**:

1. **Given** a first-time user opens the app, **When** they provide the required
   body metrics and goal-related inputs, **Then** the app calculates and stores a
   daily calorie budget and shows it as the baseline for today's tracking.
...
- What happens when the user changes profile information that affects the calorie
  budget after several days of existing entries?
...
- **FR-001**: The system MUST onboard first-time users by asking for the minimum
  personal details needed to estimate a daily calorie budget.
- **FR-002**: The system MUST calculate a daily calorie budget from onboarding data
  using a recognized calorie-needs formula and store the resulting budget.
...
- **FR-006**: The system MUST record food entries silently by default, without
  ...
  workflows for the primary logging path.
...
- **User Profile**: The personal information used to estimate calorie needs, such as
  body metrics, age-related data, activity assumptions, and weight-loss goal inputs.
